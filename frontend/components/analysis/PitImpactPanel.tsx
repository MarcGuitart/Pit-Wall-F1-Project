...
function PositionDeltaBadge({ delta }: { delta: number | null }) {
  ...
  if (delta === 0)
    return (
      <span className="font-mono font-bold text-[16px] text-text-secondary">±0</span>
    )
  if (delta > 0)
    return (
      <span className="font-mono font-bold text-[16px] text-signal-green">+{delta}</span>
    )
  return (
    <span className="font-mono font-bold text-[16px] text-signal-red">{delta}</span>
  )
}
...
export function PitImpactPanel({ rows }: Props) {
  return (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3">
                <div>
                  ...
                  <span className="font-display text-[8px] uppercase text-text-muted ml-1">
                    lane
                  </span>
                </div>
